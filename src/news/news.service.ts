...
export class NewsService {
  ...
  async createNews(
    ...
    const newsFromDb = await this.newsRepository.findOne({
    ...
    if (news.authorId === user.id || user.role === UserRoleEnum.ADMIN) {
      ...
      if (!updatedNews.affected) {
        throw new NotFoundException(
          `News with slug "${slug}" was not updated!`,
        );
      }
      if (images && images.images) {
        const imagesOnDelete = await this.imageRepository.find({
        ...
        try {
          ...
          console.log('Images for delete not found!');
        }
  ...
  async deleteNews(slug: string, user: UserEntity) {
    ...
    if (news.authorId === user.id || user.role === UserRoleEnum.ADMIN) {
      ...
      if (!result.affected) {
        throw new NotFoundException(
          `News with slug "${slug}" was not deleted!`,
        );
      }
      if (imagesOnDelete) {
        for (const image of imagesOnDelete) {
  ...
  async unlikeNews(newsSlug: string, user: UserEntity) {
    const news = await this.findNewsBySlug(newsSlug);
    news.likedByUsers = news.likedByUsers.filter(

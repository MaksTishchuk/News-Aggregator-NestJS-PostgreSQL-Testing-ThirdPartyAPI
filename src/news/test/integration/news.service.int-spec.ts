...
describe('NewsService Int', () =>  {
  ...
  describe('createNews', () => {
    ...
    it('should create user', async () => {
      user = await userRepository.save({
        email: 'test-user@gmail.com', username: 'test-user', password: 'qwerty', isActivated: true
      })
    })
  ...
  describe('findAllNews', () => {
    ...
    it('should create several news for test', async () => {
      await newsService.createNews({title: 'news 2', body: 'body 2'}, user, [])
      await newsService.createNews({title: 'news 3', body: 'body 3'}, user, [])
    })
  ...
  describe('findOneNews', () => {
    ...
    it('should throw error after not found news', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(`News with slug "${testSlug}" was not found!`)
      }
    })
  })
  ...
  describe('updateNews', () => {
    ...
    it('should throw error after try to update news by another user who not author', async () => {
      notAuthorUser = await userRepository.save({
        email: 'not-author@gmail.com', username: 'not-author', password: 'qwerty', isActivated: true
      })
      try {
        await newsService.updateNews(slug, updateNewsDto, notAuthorUser, [])
      } catch (error) {
        expect(error).toBeInstanceOf(BadRequestException)
        expect(error.message).toContain(`News with slug "${slug}" was not updated! Access denied!`)
      }
    })
  })
  ...
  describe('deleteNews', () => {
    ...
    it('should throw error after try to delete news by another user who not author', async () => {
      try {
        ...
        expect(error).toBeInstanceOf(BadRequestException)
        expect(error.message).toContain(`News with slug "${slug}" was not deleted! Access denied!`)
      }
    })

    it('should delete one news', async () => {
      const result = await newsService.deleteNews(slug, user)
      expect(result).toEqual({success: true, message: 'News has been deleted!'})
    })

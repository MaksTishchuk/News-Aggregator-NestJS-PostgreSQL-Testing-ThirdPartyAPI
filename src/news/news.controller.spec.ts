...
describe('NewsController', () => {
  ...
  const mockNewsService = {
    ...
    createNews: jest
      .fn()
      .mockImplementation(
        (createNewsDto: CreateNewsDto, user: UserEntity, []) => {
          return {
            ...mockNews,
            ...createNewsDto,
            author: user,
    ...
    updateNews: jest
      .fn()
      .mockImplementation(
        (slug: string, updateNewsDto: UpdateNewsDto, user: UserEntity, []) => {
          return {
            ...mockNews,
            ...updateNewsDto,
            author: user,
    ...
    deleteNews: jest
      .fn()
      .mockImplementation((slug: string, user: UserEntity) => {
        return {
          success: true,
    ...
    likeNews: jest
      .fn()
      .mockImplementation((newsSlug: string, user: UserEntity) => {
    ...
    unlikeNews: jest
      .fn()
      .mockImplementation((newsSlug: string, user: UserEntity) => {
        mockNewsWithLike.likedByUsers = mockNewsWithLike.likedByUsers.filter(
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      .useValue(mockNewsService) // i am about it early
      .compile();
  ...
  it('should creat a news', () => {
    ...
    expect(mockNewsService.createNews).toHaveBeenCalledWith(
      createMockNewsDto,
      mockUser,
      [],
    );
  ...
  it('should skip some news and take some news', () => {
    expect(newsController.searchNews(mockSearchNewsDto)).toEqual(
      mockNewsList
        .slice(mockSearchNewsDto.skip)
        .slice(0, mockSearchNewsDto.take),
    );
  ...
  it('should update a news', () => {
    expect(
      newsController.updateNews('news-1', mockUpdateNewsDto, mockUser, []),
    ).toEqual({
      ...mockNews,
      ...mockUpdateNewsDto,
      author: mockUser,
    ...
    expect(mockNewsService.updateNews).toHaveBeenCalledWith(
      'news-1',
      mockUpdateNewsDto,
      mockUser,
      [],

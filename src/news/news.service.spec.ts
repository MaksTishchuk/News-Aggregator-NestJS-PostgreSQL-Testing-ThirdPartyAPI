...
describe('NewsService', () => {
  ...
  const mockNewsRepository = {
    ...
    update: jest.fn((slug, mockUpdateNewsDto) => Promise.resolve({affected: 1})),
  ...
  describe('findOneNews', () => {
    ...
    it('should throw error', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(`News with slug "${mockNews.slug}" was not found!`)
      }
    })
  })
  ...
  describe('followingUsersNews', () => {
    it('should find news from following user', async () => {
      jest.spyOn(mockUserRepository, 'findOne').mockImplementation(() => mockUserWithFollowers)
      jest.spyOn(lodash, 'sortBy').mockImplementation((mockNews, parameter='created') => {return mockNewsList.reverse()})
      const newsList = await newsService.followingUsersNews(mockUserWithFollowers)
    ...
    it('should throw error', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(NotFoundException)
        expect(error.message).toContain(`User with id ${mockUserWithFollowers.id} was not found!`)
      }
    })
  })
  ...
  describe('deleteNews', () => {
    it('should delete one news', async () => {
      const findOneSpy = jest.spyOn(mockNewsRepository, 'findOne')
      jest.spyOn(mockNewsRepository, 'findOne').mockImplementation(() => Promise.resolve(mockNews))
      const result = await newsService.deleteNews(mockNews.slug, mockUser)
      expect(result).toStrictEqual({success: true, message: 'News has been deleted!'})
      expect(findOneSpy).toHaveBeenCalledTimes(1)
      expect(findOneSpy).toHaveBeenCalledWith({where: {slug: mockNews.slug}, relations: ['author', 'comments', 'comments.author', 'images']})
    })
    ...
    it('should throw error news was not deleted', async () => {
      jest.spyOn(mockNewsRepository, 'findOne').mockImplementation(() => Promise.resolve(mockNews))
      mockNewsRepository.delete = jest.fn().mockResolvedValue({affected: false})
      try {
        await newsService.deleteNews(mockNews.slug, mockUser)
      } catch (error) {
        expect(error).toBeInstanceOf(NotFoundException)
        expect(error.message).toContain(`News with slug "${mockNews.slug}" was not deleted!`)
      }
    })
    ...
    it('should throw error access denied', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(`News with slug "${mockNews.slug}" was not deleted! Access denied!`)
      }
    })
  })
  ...
  describe('unlikeNews', () => {
    it('should unlike one news', async () => {
      const findOneSpy = jest.spyOn(mockNewsRepository, 'findOne')
      const saveSpy = jest.spyOn(mockNewsRepository, 'save')
      jest.spyOn(mockNewsRepository, 'findOne').mockImplementation(() => Promise.resolve(mockNewsWithLike))
      const news = await newsService.unlikeNews(mockNewsWithLike.slug, mockUser)
      expect(news).toEqual({...mockNewsWithLike, likedByUsers: []})
      expect(findOneSpy).toHaveBeenCalledTimes(2)
      expect(findOneSpy).toHaveBeenCalledWith({where: {slug: mockNewsWithLike.slug}, relations: ['author', 'comments', 'comments.author', 'images']})
      expect(saveSpy).toHaveBeenCalledTimes(1)
    })
  })

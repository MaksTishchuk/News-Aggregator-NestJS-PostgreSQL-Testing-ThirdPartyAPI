...
describe('UserService', () => {
  ...
  const mockUserRepository = {
    ...
    update: jest.fn((id, mockUpdateUserProfileDto) =>
    ...
    it('should throw user not found error', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(
          `User with id "${mockUser.id}" was not found!`,
        );
      }
    });
  });
  ...
  describe('updateUserProfile', () => {
    it('should update user profile', async () => {
      ...
      const mockUpdateUserProfileWithGenderDto = {
        ...mockUpdateUserProfileDto,
        gender: GenderEnum.MALE,
      };
      const mockUpdatedUser = {
        ...MockUser,
        ...mockUpdateUserProfileWithGenderDto,
      };
      const createQueryBuilder: any = {
        leftJoinAndSelect: () => createQueryBuilder,
        where: () => createQueryBuilder,
  ...
  describe('deleteUser', () => {
    ...
    it('should throw error on delete user', async () => {
      mockUserRepository.delete = jest
        .fn()
        .mockResolvedValue({ affected: false });
      try {
        ...
        expect(error).toBeInstanceOf(NotFoundException);
        expect(error.message).toContain(
          `User with id ${mockUser.id} was not deleted!`,
        );
      }
    });
  });

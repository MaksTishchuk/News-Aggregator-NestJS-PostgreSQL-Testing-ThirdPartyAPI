...
export class UserService {
  ...
  async updateUserProfile(
    ...
    try {
      const { firstName, lastName, phoneNumber, country, city, gender } =
      ...
      const updatedUser = await this.userRepository.update(
        { id: user.id },
        {
          ...
          lastName:
            typeof lastName !== 'undefined' ? lastName : userProfile.lastName,
          phoneNumber:
            typeof phoneNumber !== 'undefined'
              ? phoneNumber
              : userProfile.phoneNumber,
          country:
            typeof country !== 'undefined' ? country : userProfile.country,
      ...
      if (!updatedUser.affected) {
        throw new NotFoundException(
          `User with id "${user.id}" has not been updated!`,
        );
      }
  ...
  async followUser(user: UserEntity, followUserId: number) {
    ...
    if (!foundUser) {
      throw new NotFoundException(
        `User with this id ${followUserId} was not found! Follow failed!`,
      );
    }
  ...
  async unfollowUser(user: UserEntity, followUserId: number) {
    ...
    if (!foundUser) {
      throw new NotFoundException(
        `User with this id ${followUserId} was not found! Unfollow failed!`,
      );
    }

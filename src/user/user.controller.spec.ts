...
describe('UserController', () => {
  ...
  const mockUserService = {
    ...
    searchUsers: jest
      .fn()
      .mockImplementation((searchUsersDto: SearchUsersDto) => {
        return mockUserList.filter(
          (user) => user.username === searchUsersDto.username,
        );
      }),
  ...
  it('should find user with same username', () => {
    expect(userController.searchUsers(mockSearchUsersDto)).toEqual(
      mockUserList.filter(
        (user) => user.username === mockSearchUsersDto.username,
      ),
    );
  ...
  it('should get user profile', () => {
    expect(userController.getUserProfile(mockUser)).toEqual({
      ...mockUser,
      id: mockUser.id,
    });
  ...
  it('should update user profile', () => {
    expect(
      userController.updateUserProfile(mockUser, mockUpdateUserProfileDto, []),
    ).toEqual({ ...mockUser, id: mockUser.id, ...mockUpdateUserProfileDto });

...
describe('UserController Int', () => {
  ...
  describe('findAllUsers', () => {
    it('should create several users and find them', async () => {
      user = await userRepository.save({
        email: 'controller-user-1@gmail.com',
        username: 'controller-user-1',
        password: 'qwerty',
        isActivated: true,
      });
      await userRepository.save({
        email: 'controller-user-2@gmail.com',
        username: 'controller-user-2',
        password: 'qwerty',
        isActivated: true,
      });
      await userRepository.save({
        email: 'controller-user-3@gmail.com',
        username: 'controller-user-3',
        password: 'qwerty',
        isActivated: true,
      });
  ...
  describe('searchUsers', () => {
    ...
    it('should return error by empty searchUsersDto', async () => {
      ...
      expect(response.body.message).toStrictEqual(
        'Search fields should not be empty!',
      );
    });
  });
  ...
  describe('followUser', () => {
    ...
    it('should return error by user not found with fake id', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.body.message).toStrictEqual(
        `User with this id ${99999} was not found! Follow failed!`,
      );
    });
    ...
    it('should return error by try to follow by himself', async () => {
      ...
      expect(response.body.message).toStrictEqual(
        `A user cannot subscribe to himself!`,
      );
    });
  });
  ...
  describe('unfollowUser', () => {
    it('should return error by user not found with fake id', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.body.message).toStrictEqual(
        `User with this id ${99999} was not found! Unfollow failed!`,
      );
    });

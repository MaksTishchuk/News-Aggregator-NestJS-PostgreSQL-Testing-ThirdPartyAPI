...
describe('AuthService', () => {
  ...
  const mockJwtService = {
    ...
    verify: jest.fn(() => Promise.resolve({id: mockUser.id, username: mockUser.username, email: mockUser.email}))
  }
  ...
  describe('register', () => {
    it('should register user', async () => {
      ...
      jest.spyOn(bcryptjs, 'hash').mockImplementation((password, salt) => hashPassword)
      ...
      expect(createSpy).toHaveBeenCalledWith({username: mockUser.username, email:mockUser.email, password: hashPassword})
  ...
  describe('activate', () => {
    it('should activate user', async () => {
      ...
      expect(result).toStrictEqual({success: true, message: `Account with email "${mockUser.email}" has been activated!`})
    ...
    it('should throw UnauthorizedException', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(`User with activation link token "${mockAccessToken}" was not found!`)
      }
    })
  })
  ...
  describe('login', () => {
    it('should login user', async () => {
      const findOneSpy = jest.spyOn(mockUserRepository, 'findOne')
      jest.spyOn(mockUserRepository, 'findOne').mockImplementation(() => mockUser)
      ...
      expect(findOneSpy).toHaveBeenCalledWith({where: {email: mockUser.email}})
      expect(findOneSpy).toHaveBeenCalledTimes(1)
    })
  ...
  describe('changePassword', () => {
    it('should change user password', async () => {
      ...
      const hashPassword = 'hashPassword'
      jest.spyOn(bcryptjs, 'hash').mockImplementation((password, salt) => hashPassword)
      const result = await authService.changePassword(mockUser, mockChangePasswordDto)
      expect(result).toStrictEqual(
        {
          success: true,
    ...
    it('should throw InternalServerErrorException by cant update user', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(InternalServerErrorException)
        expect(error.message).toContain(`Password for user with email "${mockUser.email}" has not been updated!`)
      }
    })
  })

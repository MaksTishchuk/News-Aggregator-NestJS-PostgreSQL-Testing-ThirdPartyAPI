...
export class AuthService {
  ...
  async register(registerCredentialsDto: RegisterCredentialsDto) {
    ...
    try {
      ...
      await this.sendActivationLinkOnEmail(
        fullActivationLink,
        user.email,
        user.username,
      );
  ...
  async activate(token: string) {
    ...
    if (!user) {
      throw new UnauthorizedException(
        `User with activation link token "${token}" was not found!`,
      );
    }
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    await this.sendForgotPasswordLinkOnEmail(
      forgotPasswordLink,
      user.email,
      user.username,
    );
  ...
  async changePassword(user: UserEntity, changePasswordDto: ChangePasswordDto) {
    ...
    if (!updatedUser.affected) {
      throw new InternalServerErrorException(
        `Password for user with email "${user.email}" has not been updated!`,
      );
    }

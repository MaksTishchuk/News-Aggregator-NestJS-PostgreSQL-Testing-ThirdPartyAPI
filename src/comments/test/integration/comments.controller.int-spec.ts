...
describe('CommentsService Int', () =>  {
  ...
  describe('createComment', () => {
    ...
    it('should create user and news', async () => {
      user = await userRepository.save({
        email: 'test-com-controller-user@gmail.com', username: 'test-com-controller-user', password: 'qwerty', isActivated: true
      })
      news = await newsRepository.save({
        title: 'news controller', body: 'body', slug: 'news-controller', author: user
      })
    })
    ...
    it('should create comment', async () => {
      ...
      const response = await request(httpServer).post('/comments').send(createCommentDto)
  ...
  describe('findAllComments', () => {
    ...
    it('should create several comments for test', async () => {
      await commentRepository.save({text: 'Comment 20', slug: 'comment-20', news: news, author: user})
      await commentRepository.save({text: 'Comment 30', slug: 'comment-30', news: news, author: user})
    })
  ...
  describe('findOneComment', () => {
    ...
    it('should throw error after not found comment', async () => {
      ...
      expect(response.body.message).toStrictEqual(`Comment with id "${99999}" was not found!`)
    })
  })
  ...
  describe('updateComment', () => {
    ...
    it('should update comment', async () => {
      const response = await request(httpServer).put(`/comments/${comment.id}`).send(updateCommentDto)
      expect(response.status).toBe(200)
      expect(response.body.id).toEqual(comment.id)
    ...
    it('should throw error after update fail', async () => {
      const response = await request(httpServer).put(`/comments/${99999}`).send(updateCommentDto)
      expect(response.status).toBe(404)
      expect(response.body.message).toContain(`Comment with id "${99999}" was not updated! Access Denied!`)
    })
  })

  describe('deleteComment', () => {
    it('should delete one comment', async () => {
      const response = await request(httpServer).delete(`/comments/${comment.id}`)
      expect(response.status).toBe(200)
      expect(response.body).toStrictEqual({success: true, message: 'Comment has been deleted!'})
    })

    it('should throw error after not found comment', async () => {
      const response = await request(httpServer).delete(`/comments/${comment.id}`)
      expect(response.status).toBe(404)
      expect(response.body.message).toStrictEqual(`Comment with id "${comment.id}" was not deleted! Access Denied!`)
    })
  })
})
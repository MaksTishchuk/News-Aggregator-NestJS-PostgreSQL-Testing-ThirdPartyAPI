  ...
  describe('createComment', () => {
    it('should create user and news', async () => {
      user = await userRepository.save({
        email: 'test-comment-user@gmail.com',
  ...
  describe('findAllComments', () => {
    it('should create several comments for test', async () => {
      await commentsService.createComment(
        { text: 'Comment 2', newsSlug: news.slug },
        user,
      );
      await commentsService.createComment(
        { text: 'Comment 3', newsSlug: news.slug },
        user,
      );
    });
    ...
    it('should throw error after not found comment', async () => {
      try {
        ...
        expect(error.message).toContain(
          `Comment with id "${9999}" was not found!`,
        );
      }
    });
  });
  ...
  describe('updateComment', () => {
    ...
    it('should throw error after update fail', async () => {
      try {
        ...
        expect(error).toBeInstanceOf(NotFoundException);
        expect(error.message).toContain(
          `Comment with id "${9999}" was not updated! Access Denied!`,
        );
      }
    });
  });
  ...
  describe('deleteComment', () => {
    it('should throw error after no access user for delete', async () => {
      const createdUser = await userRepository.save({
        email: 'test-comment-user-2@gmail.com',
        username: 'test-comment-user-2',
        password: 'qwerty',
        isActivated: true,
      });
      try {

...
export class CommentsService {
  ...
  async createComment(createCommentDto: CreateCommentDto, user: UserEntity) {
    const news = await this.newsRepository.findOne({
      where: { slug: createCommentDto.newsSlug },
    });
  ...
  async updateComment(
    ...
    if (user.role === UserRoleEnum.ADMIN) {
      updateComment = await this.commentRepository.update(
        { id: id },
    ...
    if (!updateComment.affected) {
      throw new NotFoundException(
        `Comment with id "${id}" was not updated! Access Denied!`,
      );
    }
  ...
  async deleteComment(id: number, user: UserEntity) {
    ...
    if (!result.affected) {
      throw new NotFoundException(
        `Comment with id "${id}" was not deleted! Access Denied!`,
      );
    }

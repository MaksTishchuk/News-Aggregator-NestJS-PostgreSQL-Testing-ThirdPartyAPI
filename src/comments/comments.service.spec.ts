...
describe('CommentsService', () => {
  ...
  const mockCommentRepository = {
    create: jest.fn((mockCreateCommentDto, mockUser) =>
      Promise.resolve(mockComment),
    ),
    save: jest.fn(),
    find: jest.fn(() => mockCommentList),
    findOne: jest.fn((id) => mockComment),
    update: jest.fn((id, mockUpdateCommentDto, mockUser) =>
  ...
  describe('createComment', () => {
    it('should create a new comment', async () => {
      ...
      expect(findOneSpy).toHaveBeenCalledWith({
        where: { slug: mockCreateCommentDto.newsSlug },
      });
    ...
    it('should throw error on find one comment', async () => {
      ...
      try {
        ...
        expect(error.message).toContain(
          `Comment with id "${mockComment.id}" was not found!`,
        );
      }
    });
  });
  ...
  describe('updateComment', () => {
    it('should update comment', async () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(
        { id: mockComment.id, authorId: mockUser.id },
        { text: mockUpdateCommentDto.text },
      );
      expect(findOneSpy).toHaveBeenCalledTimes(1);
      expect(findOneSpy).toHaveBeenCalledWith({
    ...
    it('should throw error on update comment', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(NotFoundException);
        expect(error.message).toContain(
          `Comment with id "${mockComment.id}" was not updated! Access Denied!`,
        );
      }
    });
  ...
  describe('deleteComment', () => {
    ...
    it('should throw error on delete comment', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(NotFoundException);
        expect(error.message).toContain(
          `Comment with id "${mockComment.id}" was not deleted! Access Denied!`,
        );
      }
    });
  });
